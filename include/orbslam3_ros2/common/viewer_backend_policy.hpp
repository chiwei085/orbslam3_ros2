 ...
 * @brief Viewer backend selection policy for avoiding Pangolin Wayland teardown
 * crashes
 *
 * This policy exists to keep backend selection logic out of ROS node startup
 * code. It can be tested without creating a ROS graph, ORB-SLAM3 system, or
 * Pangolin window.
 *
 * The policy may request a process-local environment override
 * (`WAYLAND_DISPLAY=0`) so Pangolin does not select its Wayland backend in
 * affected environments. The override is applied only in the current process
 * via `setenv()`.
 */
...
/**
 * @brief Requested viewer backend mode from user-facing parameter
 * `viewer_backend`
 */
...
struct ViewerPolicyResult
{
    ...
    ViewerBackendMode mode{
        ViewerBackendMode::Auto};      // Normalized backend mode.
    std::string backend_name{"auto"};  // Canonical backend string.
    bool has_display{false};           // DISPLAY is present and non-empty.
    bool wayland_display_active{
        false};                    // WAYLAND_DISPLAY is active and not "0".
    bool xdg_wayland{false};       // XDG_SESSION_TYPE indicates wayland.
    bool detected_wayland{false};  // Any Wayland hint is present.
    bool should_set_wayland_display{
        false};  // Caller should mutate WAYLAND_DISPLAY.
    std::string
        wayland_display_override;  // Value to write into WAYLAND_DISPLAY.
    bool warn_wayland_teardown{
        false};                 // Caller should warn about teardown risk.
 ...
 * @note This function is side-effect free. It exists so tests can bypass
 * process environment reads and pass a synthetic `ViewerBackendEnv`.
 */
inline ViewerBackendEnv get_viewer_backend_env_from_process() {
    return {
 ...
 */
inline std::pair<ViewerBackendMode, std::string> parse_viewer_backend_mode(
    ...
    return {ViewerBackendMode::Auto,
            normalized == "auto" ? normalized : "auto"};
}

/**
 * @brief Compute viewer backend policy from caller intent and environment
 * snapshot
 * @param enable_pangolin Requested viewer enable flag from wrapper parameter
 * @param viewer_backend Raw backend selector string (`auto|x11|wayland`,
 * case-insensitive)
 * @param env Environment snapshot used for backend selection and fallback
 * decisions
 * @return Policy result with effective viewer enable, normalized backend,
 * warning flags, and any process-local env mutation to apply before Pangolin
 * initialization
 * @note This function does not mutate the process environment. The wrapper
 * applies the requested env change after logging and error handling.
 * @note `WAYLAND_DISPLAY=0` is used instead of unsetting `WAYLAND_DISPLAY`
 * because it was observed to be the more reliable way to block Pangolin Wayland
 * backend selection in affected environments. The mutation remains
 * process-local.
 ...
 *
 * | `enable_pangolin` | `viewer_backend` | Env signal | `effective_enable` |
 * Env mutation | | --- | --- | --- | --- | --- | | `false` | any | any |
 * `false` | none | | `true` | `auto` | no Wayland detected | `true` | none | |
 * `true` | `auto` | Wayland detected + `DISPLAY` present | `true` |
 * `WAYLAND_DISPLAY=0` | | `true` | `auto` | Wayland detected + no `DISPLAY` |
 * `false` | none | | `true` | `x11` | `DISPLAY` present | `true` |
 * `WAYLAND_DISPLAY=0` | | `true` | `x11` | no `DISPLAY` | `false` | none | |
 * `true` | `wayland` | any | `true` | none |
 *
 * `auto` disables the viewer when Wayland is detected and `DISPLAY` is missing.
 * This avoids a hard-fail path when X11/XWayland is not exported to the
 * process.
 *
 * @code
 ...
 *
 * // Wayland session with DISPLAY available -> keep viewer enabled and force
 * X11. auto a = evaluate_viewer_backend_policy( true, "auto",
 * ViewerBackendEnv{":0", "wayland-0", "wayland"});
 ...
 * @endcode
 */
inline ViewerPolicyResult evaluate_viewer_backend_policy(
    bool enable_pangolin, const std::string& viewer_backend,
    const ViewerBackendEnv& env) {
    ...
    result.detected_wayland =
        result.wayland_display_active || result.xdg_wayland;
    ...
    if (to_lower_ascii(viewer_backend) != result.backend_name) {
        // Keep parsing permissive. Wrapper can warn without changing control
        // flow.
    ...
    switch (result.mode) {
        case ViewerBackendMode::Auto:
            if (!result.detected_wayland) {
                ...
                // No X11 endpoint is available. Disable instead of forcing a
                // failing path.
        ...
        case ViewerBackendMode::X11:
            if (!result.has_display) {
                // Explicit x11 request with no DISPLAY. Disable instead of
                // hard-failing later.
                result.effective_enable = false;
                result.reason =
                    "viewer_backend=x11 but DISPLAY is empty -> disabling "
                    "viewer";
            }
            else {
                result.should_set_wayland_display = true;
 ...
 * @details
 * The only mutation currently supported is `WAYLAND_DISPLAY=<override>`, where
 * the override is usually `"0"` to prevent Wayland backend selection.
 */
inline bool apply_viewer_backend_env_policy(const ViewerPolicyResult& result) {
    if (!result.should_set_wayland_display) {
        // No mutation requested. Keep caller path simple.
        return true;
    }
    // Override only in this process. Parent shell and system session stay
    // unchanged.
    return ::setenv("WAYLAND_DISPLAY", result.wayland_display_override.c_str(),
                    1) == 0;
}
